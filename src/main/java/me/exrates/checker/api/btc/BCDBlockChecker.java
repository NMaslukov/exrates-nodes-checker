...
public class BCDBlockChecker implements ExplorerBlocksCheckerService {

    @Autowired
    Client client;

    @Value("${bcd.blocks.endpoint}")
    private String endpoint;

    @Override
    public long getExplorerBlocksAmount() {
        return new JSONObject(client.target(endpoint).request().get().readEntity(String.class)).getJSONObject("data").getJSONArray("list").getJSONObject(0).getLong("height");
    }
